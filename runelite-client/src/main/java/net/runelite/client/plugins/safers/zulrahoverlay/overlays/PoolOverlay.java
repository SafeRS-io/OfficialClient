...
public class PoolOverlay extends Overlay {
    private final Client client;
    private final ZulrahConfig config;
    private static final int HIGHLIGHTED_OBJECT_ID = 39651;

    @Inject
    public PoolOverlay(Client client, ZulrahConfig config) {
    ...
    private void renderRectangleAroundObject(Graphics2D graphics, GameObject gameObject) {
        if (gameObject.getClickbox() != null) {
            ...
            graphics.setColor(config.poolcolor()); // Use color from config
