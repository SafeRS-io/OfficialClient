...
public class TeleBoxOverlay extends Overlay {
    private final Client client;
    private final SafeRSWintertodtConfig config;
    private static final int HIGHLIGHTED_OBJECT_ID = 29156;

    @Inject
    public TeleBoxOverlay(Client client, SafeRSWintertodtConfig config) {
    ...
    private void renderRectangleAroundObject(Graphics2D graphics, GameObject gameObject) {
        if (gameObject.getClickbox() != null) {
            ...
            graphics.setColor(new Color(96,0,255)); // Use color from config
