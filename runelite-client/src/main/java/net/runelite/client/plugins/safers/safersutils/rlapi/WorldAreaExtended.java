package net.runelite.client.plugins.safers.safersutils.rlapi;
...
public class WorldAreaExtended
{
												   ...
												   Predicate<? super WorldPoint> extraCondition)
	{
		...
		if (stopAtMeleeDistance && axisX == 1 && axisY == 1)
		{
			...
			// from the target
			if (original.canTravelInDirection((WorldView) client, dxSig, 0, extraCondition))
			{
		...
		{
			if (original.canTravelInDirection((WorldView) client, dxSig, dySig, extraCondition))
			{
				return new WorldArea(x1 + dxSig, y1 + dySig, w1, h1, z1);
			}
			else if (dx != 0 && original.canTravelInDirection((WorldView) client, dxSig, 0, extraCondition))
			{
				return new WorldArea(x1 + dxSig, y1, w1, h1, z1);
			}
			else if (dy != 0 && Math.max(Math.abs(dx), Math.abs(dy)) > 1 && original.canTravelInDirection((WorldView) client, 0, dy, extraCondition))
			{
