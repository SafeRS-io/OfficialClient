...
public class ZulrahSmokeCloudsOverlay extends Overlay {
    private final Client client;
    private final ZulrahConfig config;
    private static final int HIGHLIGHTED_OBJECT_ID = 11700;

    @Inject
    public ZulrahSmokeCloudsOverlay(Client client, ZulrahConfig config) {
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        GameObject secondClosestObject = null;
                        ...
                        for (GameObject gameObject : gameObjects) {
                            if (gameObject != null && gameObject.getId() == HIGHLIGHTED_OBJECT_ID) {
                                ...
                                if (distance < closestDistance) {
                                    ...
                                    secondClosestObject = gameObject;
        ...
        if (secondClosestObject != null) {
            renderRectangleAroundObject(graphics, secondClosestObject);
        }
    ...
    private void renderRectangleAroundObject(Graphics2D graphics, GameObject gameObject) {
        if (gameObject.getClickbox() != null) {
            ...
            graphics.setColor(config.smokeColor()); // Use color from config
