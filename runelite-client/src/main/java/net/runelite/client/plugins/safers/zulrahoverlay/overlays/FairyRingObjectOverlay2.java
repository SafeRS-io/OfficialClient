...
public class FairyRingObjectOverlay2 extends Overlay {
    private final Client client;
    private final ZulrahConfig config;
    private static final int HIGHLIGHTED_OBJECT_ID = 29483;

    @Inject
    public FairyRingObjectOverlay2(Client client, ZulrahConfig config) {
    ...
    private void renderRectangleAroundObject(Graphics2D graphics, GameObject gameObject) {
        if (gameObject.getClickbox() != null) {
            ...
            graphics.setColor(config.BoatColor()); // Use color from config
