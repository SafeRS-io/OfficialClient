...
public class HighlightBigDoor extends Overlay {
    private final Client client;
    private final SafeRSWintertodtConfig config;
    private static final int HIGHLIGHTED_OBJECT_ID = 29322;

    @Inject
    public HighlightBigDoor(Client client, SafeRSWintertodtConfig config) {
    ...
    private void renderRectangleAroundObject(Graphics2D graphics, GameObject gameObject) {
        if (gameObject.getClickbox() != null) {
            ...
            graphics.setColor(config.DoorColor()); // Use color from config
